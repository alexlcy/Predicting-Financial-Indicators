...
def CalcPricetoEarningsRatio(CurrentSharePrice, EPS):
    Output = CurrentSharePrice / EPS;
    return Output;

PricetoEarningsRatio = CalcPricetoEarningsRatio(df['Stock Price'], df['EPS'])

#Split df to be target and feature variable
X = df.drop(['Date', 'Stock Price', 'EPS'], axis = 1).values
y = PricetoEarningsRatio
