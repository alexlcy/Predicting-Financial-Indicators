...
#import the dataset
df = pd.read_csv('FYP_data2.csv')

def CalcTier1CapitalRatio(Tier1Capital, RiskWeightedAssets):
    Output = Tier1Capital / RiskWeightedAssets;
    return Output;

Tier1CapitalRatio = CalcTier1CapitalRatio(df['Tier 1 Cap'], df['Risk Weighted Assets'])

#Split df to be target and feature variable
X = df.drop(['Date', 'Tier 1 Cap', 'Risk Weighted Assets'], axis = 1).values
y = Tier1CapitalRatio
