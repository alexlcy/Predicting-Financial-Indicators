...
def CalcCosttoIncomeRatio(InterestIncome, NonInterestIncome, Expenses):
    Output = Expenses / (InterestIncome + NonInterestIncome);
    return Output;

CosttoIncomeRatio = CalcCosttoIncomeRatio(df['Interest Income'], df['Non Interest Income'], df['Intereset Expenses'],)

#Split df to be target and feature variable
X = df.drop(['Date', 'Interest Income', 'Intereset Expenses', 'Non Interest Income'], axis = 1).values
y = CosttoIncomeRatio
