...
def CalcDividendPayoutRatio(DividendperShare, EPS):
    Output = DividendperShare / EPS;
    return Output;

DividendPayoutRatio = CalcDividendPayoutRatio(df['DPS'], df['EPS'])

#Split df to be target and feature variable
X = df.drop(['Date', 'DPS', 'EPS'], axis = 1).values
