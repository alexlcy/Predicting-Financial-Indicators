...
#Setting up ratio variables
def CalcNetInterestMargin(InterestIncome, InterestExpense, EarningAssetsAverage):
    Output = (InterestIncome - InterestExpense) / EarningAssetsAverage;
    return Output;

NetInterestMargin = CalcNetInterestMargin(df['Interest Income'], df['Intereset Expenses'], df['Earnings Assets'])

#Split df to be target and feature variable
X = df.drop(['Date', 'Interest Income', 'Intereset Expenses', 'Earnings Assets'], axis = 1).values
y = NetInterestMargin
