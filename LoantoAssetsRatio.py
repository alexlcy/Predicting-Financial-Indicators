...
def CalcLoantoAssetsRatio(Loan, Assets):
    Output = Loan / Assets;
    return Output;

LoantoAssetsRatio = CalcLoantoAssetsRatio(df['Total Loan'], df['Total Asset'])

#Split df to be target and feature variable
X = df.drop(['Date', 'Total Loan', 'Total Asset'], axis = 1).values
y = LoantoAssetsRatio
