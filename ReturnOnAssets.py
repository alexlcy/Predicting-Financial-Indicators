...
def CalcReturnOnAssets(NetIncome, AssetsAverage):
    Output = NetIncome / AssetsAverage;
    return Output;

ReturnOnAssets = CalcReturnOnAssets(df['Net Income'], (df['Total Asset']/len(df['Total Asset'])))

#Split df to be target and feature variable
X = df.drop(['Date', 'Net Income', 'Total Asset'], axis = 1).values
y = ReturnOnAssets
