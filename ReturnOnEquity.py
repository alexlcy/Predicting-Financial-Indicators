...
def CalcReturnOnEquity(NetIncome, EquityAverage):
    Output = NetIncome / EquityAverage;
    return Output;

ReturnOnEquity = CalcReturnOnEquity(df['Net Income'], (df['Total Equity']/len(df['Total Equity'])))

#Split df to be target and feature variable
X = df.drop(['Date', 'Net Income', 'Total Equity'], axis = 1).values
y = ReturnOnAssets
